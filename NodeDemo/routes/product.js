...
router.get('/', async function (req, res, next) {
  try {
    var allProducts = await modelProduct.getAllProducts(req.query);
...
router.get("/:id", async function (req, res, next) {
  try {
    var department = await modelProduct.getProductById(req.params.id);
    if (!department) {
...
router.post("/add", validate.validate(), async function (req, res, next) {
  try {
    const { name, price, order , category_k} = req.body;
    ...
    const newProducts = await modelProduct.createProduct({
      name: name,
      price: price,
      order: order,
...
router.put("/edit/:id", validate.validate(["name"]), async function (req, res, next) {
  try {
    ...
    const updatedProducts = await modelProduct.updateProduct(
      req.params.id,
      req.body
    );
...
router.delete("/delete/:id", async function (req, res, next) {
  try {
    ...
    var deletedProducts = await modelProduct.deleteProduct(idDelete);
